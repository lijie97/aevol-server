...
ae_individual* param_loader::create_random_individual( ae_exp_manager* exp_m, ae_params_mut* param_mut, int32_t id ) const
{
  ...
  ae_genetic_unit * chrom = new ae_genetic_unit( indiv, _chromosome_initial_length); // a random sequence is generated by the ae_string constructor
  indiv->add_GU( chrom );
  ...
  if (_allow_plasmids) // We create a plasmid
  {
    ...
    {
      ae_genetic_unit * plasmid = new ae_genetic_unit( indiv, _plasmid_initial_length); // a random sequence is generated by the ae_string constructor
...
ae_individual* param_loader::create_random_individual_with_good_gene( ae_exp_manager* exp_m, ae_params_mut* param_mut, int32_t id ) const
{
  ...
  indiv->add_GU( chrom );
  indiv->evaluate( exp_m->get_env() );
  printf("%f %f\n", indiv->get_genetic_unit(0)->get_dist_to_target_by_feature( METABOLISM ), indiv->get_dist_to_target_by_feature( METABOLISM ));

  while ( indiv->get_dist_to_target_by_feature( METABOLISM ) >= env_metabolic_area )
    {
      indiv->remove_GU(0);
      chrom = new ae_genetic_unit( indiv, _chromosome_initial_length); // a random sequence is generated by the ae_string constructor
      indiv->add_GU( chrom );
      indiv->evaluate( exp_m->get_env() );
      printf("%f %f\n", indiv->get_genetic_unit(0)->get_dist_to_target_by_feature( METABOLISM ), indiv->get_dist_to_target_by_feature( METABOLISM ));
    }
      ...
      if ( _plasmid_initial_gene == 1 )
        {
           ...
           while ( indiv->get_genetic_unit(1)->get_dist_to_target_by_feature( METABOLISM ) >= env_metabolic_area )
             {
               indiv->remove_GU(1);
               plasmid = new ae_genetic_unit( indiv, _plasmid_initial_length); // a random sequence is generated by the ae_string constructor
               indiv->add_GU( plasmid );
               indiv->evaluate( exp_m->get_env() );
             }
        }
