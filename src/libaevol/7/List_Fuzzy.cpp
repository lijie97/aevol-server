...
/// functions.
void List_Fuzzy::add(const AbstractFuzzy& f) {
  ...
  // assert(invariant());
    ...
    void List_Fuzzy::add(List_Fuzzy* f) {
        ...
        // each point in `points_` gets `fs`'s counterpart ordinate added to
        // it.
        for (std::list<Point>::iterator p = points_.begin(); p != points_.end(); ++p)
            (*p).y += f->y((*p).x);
...
void List_Fuzzy::sub(const AbstractFuzzy& f, bool verbose) {
  const List_Fuzzy fs = (List_Fuzzy&)(f);
  // assert(invariant());
  ...
  // assert(invariant());
    ...
    void List_Fuzzy::sub(List_Fuzzy* f, bool verbose) {
      ...
      // if (verbose) print();
        ...
        for (const Point q: f->points_) {
            ...
            begin = create_interpolated_point(q.x,begin,verbose);
            (*begin).y += q.y;
            look.insert(q.x);
        }
...
bool List_Fuzzy::is_identical_to(const AbstractFuzzy& f, ProteinConcentration tolerance ) const {
  const List_Fuzzy fs = (List_Fuzzy&)(f);
