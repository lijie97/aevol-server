...
void transfer_in(ExpManager* exp_m, bool first_gen) {
    ...
    std::vector<size_t> host_dna_size(exp_m->nb_indivs());
    std::vector<size_t> host_dna_offset(exp_m->nb_indivs());
    ...
    int width_ = exp_m->world()->width();
    int height_ = exp_m->world()->height();
    ...
    for (int16_t x = 0; x < width_; x++)
        for (int16_t y = 0; y < height_; y++) {
            ...
            global_dna_size += exp_m->world()->indiv_at(x,y)->genetic_unit(0).seq_length();
            host_max_dna_size =
                    host_max_dna_size < exp_m->world()->indiv_at(x,y)->genetic_unit(0).seq_length() ?
                    exp_m->world()->indiv_at(x,y)->genetic_unit(0).seq_length() : host_max_dna_size;
            host_dna_size[i] = exp_m->world()->indiv_at(x,y)->genetic_unit(0).seq_length();
        }

  // Create shorthands
  auto seq0 = exp_m->world()->indiv_at(0,0)->genetic_unit(0).dna()->data();
  auto len0 = exp_m->world()->indiv_at(0,0)->genetic_unit(0).seq_length();
  ...
  checkCuda(cudaMemcpy(dna_size,
                       host_dna_size.data(), exp_m->nb_indivs() * sizeof(size_t),
                       cudaMemcpyHostToDevice));
  ...
  int y_dim_size = exp_m->nb_indivs();
  ...
  checkCuda(cudaMalloc((void**) &nb_mutations,
                       (exp_m->nb_indivs() + 1) * sizeof(int)));
  checkCuda(cudaMemset(nb_mutations, 0, (exp_m->nb_indivs() + 1) * sizeof(int)));

  checkCuda(cudaMalloc((void**) &mutations_offset,
                       exp_m->nb_indivs() * sizeof(int)));
  checkCuda(cudaMemset(mutations_offset, 0, exp_m->nb_indivs() * sizeof(int)));

  checkCuda(cudaMalloc((void**) &mutations_idx,
                       exp_m->nb_indivs() * sizeof(int)));
  checkCuda(cudaMemset(mutations_idx, 0, exp_m->nb_indivs() * sizeof(int)));

    checkCuda(cudaMalloc((void**) &nb_diff_mutations,
                         (exp_m->nb_indivs() + 1) * sizeof(int)));
    checkCuda(cudaMemset(nb_diff_mutations, 0, (exp_m->nb_indivs() + 1) * sizeof(int)));

    checkCuda(cudaMalloc((void**) &diff_mutations_offset,
                         exp_m->nb_indivs() * sizeof(int)));
    checkCuda(cudaMemset(diff_mutations_offset, 0, exp_m->nb_indivs() * sizeof(int)));


  checkCuda(cudaMalloc((void**) &dna_mutator_list,
                       exp_m->nb_indivs() * sizeof(GPUDnaMutator)));

  current_size_tab_mutation = exp_m->nb_indivs() * 100;
  checkCuda(cudaMalloc(&tab_mutation,
                       current_size_tab_mutation * sizeof(TypeMutation)));


    current_size_tab_diff_mutation = exp_m->nb_indivs() * 100;
    checkCuda(cudaMalloc(&tab_diff_mutation,
                         current_size_tab_diff_mutation * sizeof(int)));

  checkCuda(cudaMalloc((void**) &rna_idx,
                       (exp_m->nb_indivs() + 1) * sizeof(int32_t)));
  checkCuda(cudaMemset(rna_idx, 0, (exp_m->nb_indivs() + 1) * sizeof(int32_t)));

  checkCuda(cudaMalloc((void**) &rna_offset,
                       exp_m->nb_indivs() * sizeof(int32_t)));
  checkCuda(cudaMemset(rna_offset, 0, exp_m->nb_indivs() * sizeof(int32_t)));

  checkCuda(cudaMalloc((void**) &protein_idx,
                       (exp_m->nb_indivs() + 1) * sizeof(int32_t)));
  checkCuda(
      cudaMemset(protein_idx, 0, (exp_m->nb_indivs() + 1) * sizeof(int32_t)));

  checkCuda(cudaMalloc((void**) &protein_offset,
                       exp_m->nb_indivs() * sizeof(int32_t)));
  checkCuda(cudaMemset(protein_offset, 0, exp_m->nb_indivs() * sizeof(int32_t)));
  ...
  checkCuda(cudaMalloc((void**) &nb_promoters,
                       (exp_m->nb_indivs() + 1) * sizeof(int)));
  checkCuda(cudaMemset(nb_promoters, 0, (exp_m->nb_indivs() + 1) * sizeof(int)));

  checkCuda(cudaMalloc((void**) &nb_proteins,
                       (exp_m->nb_indivs() + 1) * sizeof(int)));
  checkCuda(cudaMemset(nb_proteins, 0, (exp_m->nb_indivs() + 1) * sizeof(int)));

  host_phenotype = (double**) malloc(exp_m->nb_indivs() * sizeof(double*));
  checkCuda(
      cudaMalloc((void***) &phenotype, exp_m->nb_indivs() * sizeof(double*)));


  host_phenotype_activ = (double**) malloc(exp_m->nb_indivs() * sizeof(double*));
  checkCuda(cudaMalloc((void***) &phenotype_activ,
                       exp_m->nb_indivs() * sizeof(double*)));


  host_phenotype_inhib = (double**) malloc(exp_m->nb_indivs() * sizeof(double*));
  checkCuda(cudaMalloc((void***) &phenotype_inhib,
                       exp_m->nb_indivs() * sizeof(double*)));

  for (int indiv_id = 0; indiv_id < exp_m->nb_indivs(); indiv_id++) {
    checkCuda(
  ...
  current_size_rna_list = exp_m->nb_indivs() * 10000;
  checkCuda(cudaMalloc(&rna, current_size_rna_list * sizeof(cuRNA)));

  current_size_protein_list = exp_m->nb_indivs() * 1000;
  checkCuda(cudaMalloc(&protein, current_size_protein_list * sizeof(cuProtein)));

  checkCuda(
      cudaMemcpy(phenotype, host_phenotype, exp_m->nb_indivs() * sizeof(double*),
                 cudaMemcpyHostToDevice));
  ...
  for (int i = 0; i < 300; i++) {
      PhenotypicTargetHandler* pth = exp_m->world()->phenotypic_target_handler();
      Fuzzy* fuz = (Fuzzy*) pth->phenotypic_target().fuzzy();
      //->phenotypic_target()->fuzzy();

    target_host[i] =  fuz->y(((double)i)/300.0);
  }
...
__global__
void fill_RNA( int8_t* dna_term, size_t* dna_size, size_t* dna_offset, int* nb_promoters, int* rna_offset, cuRNA* rnas,
...
__global__
void compute_RNA( int8_t* dna_term, size_t* dna_size, size_t* dna_offset, cuRNA* rnas,  int global_nb_rna) {
...
__global__ void display_RNA( cuRNA* rna, size_t* dna_size, int32_t global_nb_rna) {
...
__global__
void compute_start_protein(int8_t* start_protein, size_t* dna_size, size_t* dna_offset, cuRNA* rna, char* dna, int32_t* nb_proteins,
...
__global__ void fill_protein(int8_t* start_protein, size_t* dna_offset, int* protein_idx,
                             int* protein_offset, cuRNA* rna, cuProtein* protein, size_t* dna_size,
                             int32_t global_nb_rna, int nb_indiv) {
	const int globalIdx = blockIdx.x*blockDim.x+threadIdx.x;
...
__global__
void compute_proteins( int8_t* start_protein, size_t* dna_size, size_t* dna_offset, cuProtein* protein, char* dna,
...
__global__
void translate_proteins( cuProtein* protein, size_t* dna_size, char* dna,  size_t* dna_offset, int32_t global_nb_protein, double w_max) {
    __shared__ int next_protein_idx;
    if (threadIdx.x == 0) {
        next_protein_idx = 0;
    }
...
__global__
void compute_phenotype( cuProtein* protein, int32_t global_nb_protein, double** phenotype,
...
//    compute_nb_diff_mutation_offset<<nb_indiv,128>>(nb_diff_mutations,diff_mutations_offset);
    ...
    do_memset<<<block,32>>>(phenotype_activ,phenotype_inhib,nb_mutations,nb_diff_mutations,rna_idx,protein_idx,nb_proteins,
...
__global__
void do_memset(double** phenotype_activ, double** phenotype_inhib, int* nb_mutations,
               int* nb_diff_mutations, int32_t* rna_idx,
