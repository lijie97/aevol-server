...
void GeneticUnit::do_transcription() {
  ...
  if (genome_length < PROM_SIZE) {
    ...
    //printf("[CPU] ======= BEFORE_RNA :: DO_TRANSCRIPTION ============\n");
    //display(false);
        ...
        /*if ((indiv()->grid_cell_->x() == 30 && indiv()->grid_cell_->y() == 9 && AeTime::time()==136)||
         (indiv()->grid_cell_->x() == 27 && indiv()->grid_cell_->y() == 13 && AeTime::time()==27)) {
          ...
          if (transcript_start + (strand_id == LEADING ? i : -i) == 7) {
            ...
            for (int16_t x_i = 0; x_i < TERM_STEM_SIZE; x_i++) {
              printf("Search terminator (%d) : %c : %c (%d : %d)\n",pos,genome[Utils::mod(pos + x_i, len)],
                  genome[Utils::mod(pos + (TERM_SIZE - 1) - x_i, len)],Utils::mod(pos + x_i, len),
                     Utils::mod(pos + (TERM_SIZE - 1) - x_i, len));
            }
          }
        }*/
...
//          }
    ...
    //printf("[CPU] ======= AFTER_RNA :: DO_TRANSCRIPTION ============\n");
    //display(false);
}
...
void GeneticUnit::do_translation() {
  ...
  int32_t genome_length = dna_->length();
    ...
    //printf("[CPU]======= BEFORE_PROTEIN :: DO_TRANSLATION ============\n");
    //display(false);
...
//   }
    ...
    //printf("[CPU] ======= AFTER_PROTEIN :: DO_TRANSLATION ============\n");
    //display(false);
}
...
//    #pragma vector always
  ...
  /*if (AeTime::time() > 2)
    if (indiv()->grid_cell()->x()==25 && indiv()->grid_cell()->y() == 10 && pos == 10) {
      printf("Promoter found at %d : %d\n",pos,dist);
    }*/
  ...
  else // (strand == LAGGING)
  {
    ...
    for (int8_t i = 0; i < SHINE_DAL_SIZE; i++) {
      /*if (indiv()->grid_cell_->x() == 28 &&
          indiv()->grid_cell_->y() == 19) {
        printf("%c (%d)",genome[Utils::mod((pos - i),len)],Utils::mod((pos - i),len));
      }*/
      if (genome[Utils::mod((pos - i),len)] ==
...
void GeneticUnit::clear_transcribed_proteins() {
  for (auto strand: {LEADING, LAGGING}) {
    ...
    void GeneticUnit::display(bool check_to_delete) {
      bool error = false;
      printf("[CPU] -- %d -- [ BEGIN -------------------------------------------------------]\n",indiv()->grid_cell()->x()*indiv()->exp_m()->world()->height()+indiv()->grid_cell()->y());
      printf("[CPU] -- Length %d\n",dna_->length());
      printf("[CPU] -- Promoters Start : ");
      ...
      for (auto& rna : rna_list_) {
        for (auto &prom: rna) {
          printf("[%d => %d]", prom.promoter_pos(), prom.last_transcribed_pos());
        }
      }
      ...
      for (auto &strand : protein_list_) {
        for (auto &prot: strand) {
          printf("[%d => %d]", prot.shine_dal_pos(), prot.last_STOP_base_pos());
        }
      }
