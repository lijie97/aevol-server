...
ae_list<ae_point_2d*>* fuzzy::get_points() {
  return _points;
}
...
double fuzzy::get_x(double y, ae_point_2d* left_point, ae_point_2d* right_point) const {
...
ae_list_node<ae_point_2d*>* fuzzy::create_interpolated_point(double x) {
...
bool fuzzy::is_identical_to(const fuzzy * other, double tolerance ) const {
  ...
  while (ok && (point_node != NULL)) {
    ...
    if (fabs(point->x - other_point->x)> tolerance*(fabs(point->x) + fabs(other_point->x)) || fabs(point->y - other_point->y)>tolerance*(fabs(point->y) + fabs(other_point->y))) {ok = false;}
...
//                           Protected Methods
// =================================================================
double fuzzy::get_y(double x, ae_list_node<ae_point_2d*>* list_entry) const {
...
ae_list_node<ae_point_2d*>* fuzzy::create_interpolated_point(double x, ae_list_node<ae_point_2d*>* list_entry) {
  if (list_entry == NULL) list_entry = _points->get_first();
